...
2. **Dataset manifest added (closed)**
   - `data/manifests/stss_test_2.json` now tracks STSS-Test-2 file names, sizes, and checksums.
   - `python src/data/verify_data_manifest.py` validates local files against the manifest.
...
1. **SSC/LLaMA wrappers added (closed)**
   - `src/runners/run_ssc_baseline.py` and `src/runners/run_llama_baseline.py` now provide Phase 4-style wrapper entrypoints with repro artifacts.
   - Impact reduced: cross-track run metadata is now standardized.

2. **End-to-end entrypoint added (closed)**
   - `src/runners/reproduce_stss_test_2.py` orchestrates validation and run steps and writes a reproduction manifest.
...
2. Keep `data/manifests/stss_test_2.json` updated when data assets change.
3. Keep `src/runners/run_ssc_baseline.py` and `src/runners/run_llama_baseline.py` aligned with upstream argument changes.
...
7. Keep `src/runners/reproduce_stss_test_2.py` as the canonical run-manifest entrypoint.
