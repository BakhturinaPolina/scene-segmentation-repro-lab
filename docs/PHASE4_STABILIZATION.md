...
This document operationalizes Phase 4 from `docs/PROJECT_PLAN.md`.
...
Expected artifacts in `outputs/prompting/<today>/baseline_qwen3/`:
...
Expected artifacts in `outputs/prompting/<today>/<run_slug>/`:
...
Expected artifacts in `outputs/prompting/<today>/`:
...
4. **Upstream dependency sensitivity**:
   Compatibility still relies on pinned versions documented in `docs/ENVIRONMENT_SETUP.md` and related run notes.
