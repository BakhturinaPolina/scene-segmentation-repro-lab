...
| Metric | Full corpus complete (2026-07-04) |
|--------|-----------------------------------|
| Books complete | **327 / 327** |
| Sentences complete | **120,369 / 120,369** |
| Cumulative cost | ~**$514** USD (Wave 7 $513.81 + ~$0.5 retry passes) |
| Parse OK (all books, `predictions.jsonl`) | **120,369 / 120,369 (100%)** |
| Parse OK (Wave 7 books only, at wave end) | 11,823 / 11,846 (99.81%) |
| Books with remaining parse fails | **0** |
| Manual-fix rows (`manual_fix` set) | **40** |

Pilot (3 books) + Wave 1 (15) + Wave 2 (55, `dprose_161` … `dprose_692`) + Wave 3 (51, `dprose_693` … `dprose_1029`) + Wave 4 (55, `dprose_1040` … `dprose_1535`) + Wave 5 (59, `dprose_1537` … `dprose_1970`) + Wave 6 (58, `dprose_1983` … `dprose_2312`) + **Wave 7 (31, `dprose_2317` … `dprose_2505`)**.

### Parse failures — corpus inventory (post-remediation)

Authoritative source: per-book `predictions.jsonl` on disk (2026-07-04). After Wave 7 completion there were **130** failed keys across **75** books. Corpus-wide remediation (see [Parse failure remediation](#parse-failure-remediation-2026-07-04)) cleared **90** keys:

| Pass | Log | Keys before → after | Recovered |
|------|-----|---------------------|-----------|
| Prior partial retries (2026-06-30) | `retry_failed_2026-06-30*.log` | — | 44 |
| Batch `--retry_failed` (2026-07-04) | `retry_failed_2026-07-04.log` | 130 → 58 | 72 |
| Sync API retry (2026-07-04) | `sync_retry_failed_2026-07-04.log` | 58 → 40 | 18 |
| Neighbor-consensus patch (applied 2026-07-04) | `patch_suggestions.{json,csv}` | 40 → **0** | 40 (37 medium+, 3 low on 2nd pass) |

All **327** books pass the **≥95% parse gate**. **100% parse OK** achieved via neighbor-consensus patch (40 keys, all `manual_fix`).

<details>
<summary>Parse-failure keys patched (40 total — audit via <code>manual_fix</code> in predictions)</summary>

All were API safety blocks or transient 503s; labels assigned by neighbor consensus (39 NOBORDER, 1 BORDER at `dprose_2177:18`). Worst-affected books before patch: `dprose_2234` (7), `dprose_555` (6), `dprose_435` (5), `dprose_2443` (4).
